...
if not exist "%BAT_FILE%" (
    ...
    echo.
    echo Убедитесь, что все файлы находятся в:
    echo %SCRIPT_DIR%
    pause
    exit /b 1
)
...
REM Создаем bat-файл в папке автозагрузки с абсолютным путем к Program Files
(
    echo @echo off
    echo REM Автозапуск автоматизации оператора ДБО
    echo REM Создан автоматически установщиком
    echo REM Все скрипты находятся в C:\Program Files\operator_automatization-main\1\
    echo.
    echo REM Устанавливаем кодировку UTF-8
    echo chcp 65001 ^>nul 2^>^&1
    echo.
    echo REM Абсолютный путь к директории скрипта в Program Files
    echo set "SCRIPT_DIR=C:\Program Files\operator_automatization-main\1"
    echo.
