...
REM Устанавливаем кодировку UTF-8 для правильного отображения русских символов
chcp 65001 >nul 2>&1

REM Устанавливаем переменную окружения для Python (UTF-8)
...
if %ERRORLEVEL% NEQ 0 (
    ...
    if %ERRORLEVEL% NEQ 0 (
        if "%1"=="HIDDEN" (
            echo [ERROR] Python не найден в PATH! >> "%LOG_FILE%"
        ) else (
...
if not exist "%SCRIPT_PATH%" (
    if "%1"=="HIDDEN" (
        echo [ERROR] Скрипт не найден: %SCRIPT_PATH% >> "%LOG_FILE%"
    ) else (
...
if "%1"=="HIDDEN" (
    ...
    REM Используем -u для unbuffered output и явно указываем кодировку
    "%PYTHON_CMD%" -u "%SCRIPT_PATH%" >> "%LOG_FILE%" 2>&1
    ...
    REM Используем -u для unbuffered output
