...
REM Если запущен напрямую (не через VBS), проверяем параметр скрытого режима
if "%1"=="HIDDEN" (
    REM Скрытый режим - убираем заголовок и цвет
    title 
) else (
...
:LOOP
    ...
    if "%1"=="HIDDEN" (
        REM В скрытом режиме - всегда перезапускаем
    ...
    REM Если код выхода 0 - нормальное завершение (Ctrl+C) - только в видимом режиме
    if %EXIT_CODE% EQU 0 (
