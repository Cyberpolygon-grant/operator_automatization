...
class SSHTunnel:
    ...
    def start(self):
        ...
        try:
            ...
            # -L local_port:remote_host:remote_port - проброс порта
            # -N - не выполнять команды, только проброс
            # -f - запустить в фоне
            # -o StrictHostKeyChecking=no - не проверять ключи хоста
            # -o UserKnownHostsFile=/dev/null - не сохранять ключи
            ssh_cmd = [
                'ssh',
                ...
                '-o', 'UserKnownHostsFile=/dev/null' if platform.system() != 'Windows' else 'UserKnownHostsFile=NUL',
            ...
            if platform.system() == 'Windows':
                # На Windows используем subprocess без -f флага
                logger.info("   Запуск SSH туннеля (Windows)...")
                self.process = subprocess.Popen(
                    ssh_cmd[:-1],  # Убираем -f, так как на Windows он может не работать
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
                    creationflags=subprocess.CREATE_NO_WINDOW if hasattr(subprocess, 'CREATE_NO_WINDOW') else 0
                )
            else:
                # На Linux/Mac используем стандартный подход
...
class DBOOperatorAutomationSSH:
    ...
    def __init__(self, email_address, password, ssh_host, ssh_user, ssh_port=22, 
